...
object InlineCommentEndPattern : StandardPattern<MultiLineCommentStyle>(
    ...
    override fun getCommentRegexPattern(style: MultiLineCommentStyle, text: String): RegexPattern =
        buildRegexPattern {
            append(RegexLookbehind.of(RegexNonWhitespace))
            append(Constants.Char.SPACE)
            append(style.getCommentSuffix(text))
        }

    override fun isStartedBy(startable: Startable) = startable is InlineCommentStartPattern
}
